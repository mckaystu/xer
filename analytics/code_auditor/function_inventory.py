...
def build_inventory(units: Iterable[CodeUnit]) -> list[FunctionRecord]:
    ...
    for unit in units:
        for name, fn_body, start_line, display_body in extract_functions_from_unit(unit):
            ...
            # Pure ODA auto-lifecycle: no lotus.domino mix and no manual recycle → not HE risk
            if (
...
def run_function_inventory(
    ...
    *,
    ...
    for false positives (ODA auto-lifecycle, caller-owned handles, mis-hits, etc.).
    Manual .recycle() on ODA wrappers remains a CRITICAL deadlock signal (never FP).
    """
